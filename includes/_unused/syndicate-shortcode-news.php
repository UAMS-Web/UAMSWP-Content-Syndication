...
class UAMS_Syndicate_Shortcode_News extends UAMS_Syndicate_Shortcode_Base {
	...
	public function display_shortcode( $atts ) {
		...
		if ( $content = $this->get_content_cache( $atts, 'uamswp_news' ) ) {
			return apply_filters( 'uamswp_content_syndicate_news', $content, $atts );
		}
		...
		if ( 'local' === $request['scheme'] ) {
			...
			if ( 200 !== $response->get_status() ) {
				error_log( 'UAMSWP Content Syndicate: Local request error ' . absint( $response->get_status() ) . '. URL: ' . esc_url( $request_url ) );
			} else {
			...
			if ( is_wp_error( $response ) ) {
				...
				error_log( 'UAMSWP Content Syndicate: Response WP_Error. Message: ' . $response_error );
			} elseif ( 404 === wp_remote_retrieve_response_code( $response ) ) {
				error_log( 'UAMSWP Content Syndicate: Remote request 404. URL: ' . esc_url( $request_url ) );
			} else {
				...
				if ( null === $data ) {
					...
					error_log( 'UAMSWP Content Syndicate: Null JSON. Original type: ' . $original_type );
					error_log( 'UAMSWP Content Syndicate: Original URL: ' . esc_url( $request_url ) );
					error_log( 'UAMSWP Content Syndicate: Original Response Code: ' . wp_remote_retrieve_response_code( $response ) );
					$data = array();
				}
		...
		if ( 0 !== absint( $atts['local_count'] ) ) {
			...
			while ( $news_query->have_posts() ) {
				 ...
				 */
				$subset = apply_filters( 'uams_content_syndicate_local_data', $subset, $atts );
		...
		if ( 'json' === $atts['output'] ) {
			...
			?>
			<div class="uamswp-content-syndicate-wrapper">
				<ul class="uamswp-content-syndicate-list">
					<?php
					$offset_x = 0;
					foreach ( $new_data as $content ) {
						...
						?><li class="uamswp-content-syndicate-item"><a href="<?php echo esc_url( $content->link ); ?>"><?php echo esc_html( $content->title ); ?></a></li><?php
					}
					?>
				</ul>
			</div>
			...
			<div class="uamswp-content-syndicate-wrapper">
				<ul class="uamswp-content-syndicate-list">
					...
					foreach ( $new_data as $content ) {
						...
						?>
						<li class="uamswp-content-syndicate-item">
			...
			?>
			<div class="uamswp-content-syndicate-wrapper">
				<div class="uamswp-content-syndicate-container">
					<?php
					$offset_x = 0;
					foreach ( $new_data as $content ) {
						if ( $offset_x < absint( $atts['offset'] ) ) {
						...
						?>
						<div class="uamswp-content-syndicate-full">
							<span class="content-item-thumbnail">
								<?php if ( $content->full_image ) : ?><img src="<?php echo esc_url( $content->full_image ); ?>" alt="<?php echo ($content->alt_text); ?>" /><?php endif; ?>
							</span>
							<?php if ( $content->image_caption ) : ?><span class="content-item-caption wp-caption-text"><?php echo ( $content->image_caption ); ?></span><?php endif; ?>

							<span class="content-item-title"><h2><a href="<?php echo esc_url( $content->link ); ?>"><?php echo esc_html( $content->title ); ?></a></h2></span>
							<span class="content-item-byline">
								<span class="content-item-byline-date"><?php echo esc_html( date( $atts['date_format'], strtotime( $content->date ) ) ); ?></span>
								<span class="content-item-byline-author">by <?php echo esc_html( $content->author_name ); ?></span>
							</span>
		...
		if ( empty( $content ) ) {
			error_log( 'UAMSWP Content Syndicate: Empty content after output buffering.' );
		}

		// Store the built content in cache for repeated use.
		$this->set_content_cache( $atts, 'uamswp_news', $content );

		$content = apply_filters( 'uamswp_content_syndicate_json', $content, $atts );
	...
	public function process_remote_posts( $data, $atts ) {
		...
		foreach ( $data as $post ) {
			...
			if ( ! empty( $post->featured_media ) && isset( $post->_embedded->{'wp:featuredmedia'} ) && 0 < count( $post->_embedded->{'wp:featuredmedia'} ) ) {
				...
				if ( isset( $subset_feature->sizes->{'medium_large'} ) ) {
					$subset->full_image = $subset_feature->sizes->{'medium_large'}->source_url;
				} else {
					$subset->full_image = $post->_embedded->{'wp:featuredmedia'}[0]->source_url;
				}
				if ( isset( $post->_embedded->{'wp:featuredmedia'}[0]->caption) ) {
					$subset->image_caption = $post->_embedded->{'wp:featuredmedia'}[0]->caption->rendered;
				} else {
					$subset->image_caption = false;
				}
				if ( isset( $post->_embedded->{'wp:featuredmedia'}[0]->alt_text ) ) {
					$subset->alt_text = $post->_embedded->{'wp:featuredmedia'}[0]->alt_text;
				} else {
					$subset->alt_text = false;
				}
			} else {
				$subset->thumbnail = false;
				$subset->full_image = false;
			}
			 ...
			 * @param array  $atts   Attributes originally passed to the `uamswp_news` shortcode.
			 */
			$subset = apply_filters( 'uams_content_syndicate_host_data', $subset, $post, $atts );
